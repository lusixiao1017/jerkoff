...
class SmallEnemy2(pygame.sprite.Sprite):
    """
    定义小飞机敌人
    """
    energy = 1
    ...
    def __init__(self, bg_size):
        super(SmallEnemy2, self).__init__()
        self.image = pygame.image.load("material/image/enemy22.png")
        self.rect = self.image.get_rect()
        self.width, self.height = bg_size[0], bg_size[1]
        self.mask = pygame.mask.from_surface(self.image)  # 获取飞机图像的掩膜用以更加精确的碰撞检测
        self.speed = 1
...
class SmallEnemy3(pygame.sprite.Sprite):
    """
    定义小飞机敌人
    """
    energy = 1
    ...
    def __init__(self, bg_size):
        super(SmallEnemy3, self).__init__()
        self.image = pygame.image.load("material/image/enemy33.png")
        self.rect = self.image.get_rect()
        self.width, self.height = bg_size[0], bg_size[1]
        self.mask = pygame.mask.from_surface(self.image)  # 获取飞机图像的掩膜用以更加精确的碰撞检测
        self.speed = 1
        self.energy = SmallEnemy3.energy
        # 定义敌机出现的位置, 保证敌机不会在程序已开始就立即出现
        self.rect.left, self.rect.top = (
            randint(0, self.width - self.rect.width),  randint(-5 * self.rect.height, -5),
        )
...
class SmallEnemy4(pygame.sprite.Sprite):
    """
    定义小飞机敌人
    """
    energy = 1
    ...
    def __init__(self, bg_size):
        super(SmallEnemy4, self).__init__()
        self.image = pygame.image.load("material/image/enemy44.png")
        self.rect = self.image.get_rect()
        self.width, self.height = bg_size[0], bg_size[1]
        self.mask = pygame.mask.from_surface(self.image)  # 获取飞机图像的掩膜用以更加精确的碰撞检测
        self.speed = 1
        self.energy = SmallEnemy4.energy
        # 定义敌机出现的位置, 保证敌机不会在程序已开始就立即出现
        self.rect.left, self.rect.top = (
            randint(0, self.width - self.rect.width),  randint(-5 * self.rect.height, -5),
        )
